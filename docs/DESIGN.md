...
bash <(curl -fsSL https://panel.example.com/install.sh) install \
  ...
  --version 6 \
  --snell-version v6.0.0rc2          # backend injects exact "latest" for the family
...
`SNELL_V6_VERSION` (default `v6.0.0rc2`) — matching the constants in OpenSnell's installer.
...
`v5.0.1` and `v6.0.0rc2`; v6 has no armv7l build), version-branched config builder, systemd
...
Binary source per `--version`: **V6** → Surge official `v6.0.0rc2` (closed release candidate, Linux only);
...
3. **Swap the binary** to V6 (`download_surge v6.0.0rc2`); fix systemd `ExecStart`.
